...
void setup()
{
	...
	LCDInit();
	ClearLCD();
	LCDPrintString(ONE , CENTER_ALIGN, "Init");
	delay(2000);
	ClearLCD();
	...
	pinMode(GREEN_LED, OUTPUT);
	pinMode(RESET_EEPROM, INPUT);
	DBG_NL("Programma test com iniziato");
	// attachInterrupt(digitalPinToInterrupt(RESET_EEPROM), ButtonPress, RISING);
...
void TestComunication()
{
	...
	// {
	// 	SmsSended = false;
	// 	DBG_NL("Bottone permuto");
	// 	BlinkFadedLed(GREEN_LED, PWM_PERCENT(100), 25);		
	// }
	// else
	// {
		if(SignalQualityTimer.hasPassed(10000, true))
		{
			ClearLCD();
			String SigQuality = "NONE", Info = "";
			SigQuality = Gsm.signalQuality();
			Info = Gsm.getProductInfo();
			
			LCDPrintString(TWO, CENTER_ALIGN, Info.c_str());
			if(SigQuality.indexOf("99") > 0)
			{
				LCDPrintString(ONE, CENTER_ALIGN, SigQuality.c_str());
				BlinkFadedLed(RED_LED, 100, 50);
			}
			else
			{
				LCDPrintString(ONE, CENTER_ALIGN, "NONE");
			}

		}
		DBG_NL("Go");
		LCDPrintString(THREE, CENTER_ALIGN, "Test GSM");
	// }
}
